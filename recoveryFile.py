...
def changeRecipeUnitType(recipeName, changeType):
    allIng = allIngPublic.loc[allIngPublic["RecipeName"] == recipeName]
    if changeType == "Metric":
        for i in range(len(allIng)):
            if (allIngPublic.iat[i, 4] == "Imperial") and (allIngPublic.iat[i, 3] in unitNameDict):
                x = allIngPublic.iat[i, 2] * unitValsDict[allIngPublic.iat[i, 3]]
                allIngPublic.iat[i, 2] = x
                allIngPublic.iat[i, 3] = unitNameDict[allIngPublic.iat[i, 3]]
                allIngPublic.iat[i, 4] = "Metric"
    elif changeType == "Imperial":
        for i in range(len(allIng)):
            if (allIngPublic.iat[i, 4] == "Metric") and (allIngPublic.iat[i, 3] in unitNameDict):
                x = allIngPublic.iat[i, 2] * unitValsDict[allIngPublic.iat[i, 3]]
                allIngPublic.iat[i, 2] = x
                allIngPublic.iat[i, 3] = unitNameDict[allIngPublic.iat[i, 3]]
                allIngPublic.iat[i, 4] = "Imperial"
                if allIngPublic.iat[i, 3] == "tsp" and x % 3 == 0:
                    x /= 3
                    allIngPublic.iat[i, 2] = x
                    allIngPublic.iat[i, 3] = "Tbsp"
                if allIngPublic.iat[i, 3] == "oz (liquid)" and x % 8 == 0:
                    x /= 8
                    allIngPublic.iat[i, 2] = x
                    allIngPublic.iat[i, 3] = "cups (liquid)"
    if (privateRecipes.loc[(privateRecipes["RecipeName"] == recipeName), "isPublic"]).empty == False:
        print("need to change both")
    viewAll(allIngPublic)
